    ...
    try:
        ...
        if spectrum_type == 'file':
            ...
            try:
                ...
                raise ValueError(f"Error parsing file '{imported_filename_display}': {e}")
            ...
            if not np.isnan(fwhm_for_cropping) and fwhm_for_cropping > 1e-9:
                ...
                if np.sum(crop_mask) >= 2:
                    x_nm_for_calculation = x_nm_for_calculation[crop_mask]
                    y_amp_for_calculation = y_amp_for_calculation[crop_mask]
            ...
            # --- FIX IS HERE: Add the processed lambda/amplitude data to the params dictionary ---
            ...
            if lambda0_eff_nm <= 0 or bandwidth_eff_nm < 0:
                raise ValueError("Wavelength must be positive, bandwidth non-negative.")
        ...
        if 'automatic_time_range' not in form_data:
            try:
                ...
                if t_min is not None and t_max is not None and t_min < t_max:
                    ...
                    app.logger.info(f"Applying manual plot range: [{t_min}, {t_max}] fs.")
                else:
                    app.logger.warning("Manual time range invalid. Using automatic range.")
            except (TypeError, ValueError):
                app.logger.warning("Could not parse manual time range. Using automatic range.")
        else:
            app.logger.info("Using automatic plot range.")
        
        img_spec = create_matplotlib_plot(plot_spectral,
                                          lambda_nm_plot_axis=results['lambda_plot_nm_axis'],
                                          intensity_plot_data=results['spectrum_intensity_plot'],
                                          phase_plot_data=results['phase_vs_lambda_plot'],
                                          lambda0_eff_nm=lambda0_eff_nm,
                                          bandwidth_eff_nm=bandwidth_eff_nm,
                                          spectrum_source_msg=spectrum_source_msg)

        img_time = create_matplotlib_plot(plot_time_domain,
                                          t=results['t'],
                                          E_real=results['E_t_real'],
                                          I_norm=results['Intensity_t_normalized'],
                                          phase_t=results['phase_t'],
                                          inst_freq=results['inst_freq'],
                                          results=results,
                                          x_lim_manual=manual_time_limits)
        
        img_ac = create_matplotlib_plot(plot_autocorrelation,
                                        t=results['t'],
                                        autocorr_norm=results['Autocorr_t_normalized'],
                                        results=results,
                                        x_lim_manual=manual_time_limits)

        return jsonify({
            "success": True, "plot_spec_img": img_spec, "plot_time_img": img_time, "plot_ac_img": img_ac,
